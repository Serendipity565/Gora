...
type Handler struct {
	...
	// chatService 把 SSE 流式编排从 controller 抽离出来，
	// controller 只负责 HTTP 解码 / 选超时 / 包 sink，业务流转交给 service。
...
func New(registry *tool.Registry, opts ...Option) *Handler {
	...
	h.chatService = server.NewChatService(handlerAgentResolver{h: h}, h.permissionGate)
	return h
}
...
//
// service 包对 controller 类型一无所知，只需要 "按 ID 找 Agent" 这一能力。
type handlerAgentResolver struct {
	h *Handler
}

func (r handlerAgentResolver) Resolve(id string) (server.AgentRunner, bool) {
