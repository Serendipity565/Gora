...
//
// 真实实现拆分在三个子包中：
//   - model/: 领域数据类型（ChatMessage、ModelSelection 等），不带 GORM/Redis 标签。
//   - dao/  : MySQL 持久化（GORM）。提供 ModelSelectionDAO / ChatHistoryDAO / DatabaseStore + UserDAO。
//   - cache/: Redis 缓存。提供 ActiveMemoryCache。
//
// 本文件通过类型别名重新导出常用 API，便于上层（cli / app / server）以单一入口装配，
