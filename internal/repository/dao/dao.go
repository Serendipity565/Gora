...
// LocalUserID 是缺省 user_id，用于不区分用户的场景（如 CLI 启动）。
...
// NewDatabaseStoreFromGorm 把已打开的 *gorm.DB 直接包装成 DatabaseStore，
// 与 ioc.InitMysql 配合使用（cleanup 由 ioc 层负责）。
func NewDatabaseStoreFromGorm(ctx context.Context, db *gorm.DB) (DatabaseStore, error) {
	if db == nil {
		return NoopDatabaseStore{}, nil
	}
	store := &GormDatabaseStore{db: db}
...
// Close 关闭底层连接。注意：当 store 是通过 NewDatabaseStoreFromGorm 由外部 ioc 持有时，
// 调用方一般不需要手动 Close（cleanup 钩子会处理）。
...
// ListRecentMessages 按时间倒序读取最新 limit 条；返回时已按时间正序排列。
