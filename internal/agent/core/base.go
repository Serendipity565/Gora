...
// SetState 更新 Agent 状态，供具体 Agent 实现（如 EinoAgent）在生命周期内同步状态。
...
// BeginRun 为新一轮执行重置状态和停止信号，是 BaseAgent 暴露给具体实现的生命周期入口。
...
// FinishRun 结束一轮执行：复位 running、关闭 doneCh，是 BaseAgent 暴露给具体实现的生命周期出口。
