...
func TestNewRouterIsAPIOnly(t *testing.T) {
	...
	handler := server.NewHandler(tool.NewRegistry())
	router := server.NewRouter(handler, server.RouterOptions{})
...
func TestNewRouterCORSHeadersWhenEnabled(t *testing.T) {
	...
	handler := server.NewHandler(tool.NewRegistry())
	router := server.NewRouter(handler, server.RouterOptions{CORS: true})
