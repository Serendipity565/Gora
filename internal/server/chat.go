// Package service 是 Gin controller 与底层 Agent 之间的业务编排层。
//
// 这一层对 HTTP / SSE 协议无感知：controller 把请求解析后调用 service，
// service 负责协调 agent / repository / 权限 gate 等组件，并通过事件流回写。
//
...
// AgentRunner 抽象 Agent 运行能力（即可被 service 调用的最小接口）。
...
// AgentResolver 在多 Agent 场景下根据 ID 选出一个 Agent；
// id 为空时返回任一 Agent。
type AgentResolver interface {
	Resolve(id string) (AgentRunner, bool)
}

// EventSink 是 service 把 Agent 事件流回写到外部（典型为 SSE Writer）的抽象。
...
func NewChatService(resolver AgentResolver, gate *core.ToolPermissionGate) *ChatService {
	return &ChatService{
		resolver:       resolver,
		permissionGate: gate,
	}
...
// ChatRequest 是 service 内部使用的、与 HTTP 协议解耦的请求结构。
...
//   - ctx 已经携带超时；service 仅在其上叠加业务上下文（disabled tools / permission gate）。
//   - 返回的 error 通常是 ErrAgentNotFound 或 sink.WriteEvent 的错误，
//     调用方据此决定 HTTP 层的状态码。
func (s *ChatService) Stream(ctx context.Context, req ChatRequest, sink EventSink) error {
	a, ok := s.resolver.Resolve(req.AgentID)
	if !ok {
	...
	if s.permissionGate != nil {
		ctx = core.WithToolPermissionGate(ctx, s.permissionGate)
	}
