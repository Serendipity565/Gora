...
#   make wire            重新生成 internal/wired/wire_gen.go（修改 ProviderSet 后必须跑）

CONFIG ?= configs/config.yaml
ADDR   ?= :8080

.PHONY: help server chat frontend-install frontend-dev frontend-build build test wire
...
chat: ## 启动命令行交互式 Agent 对话
	go run ./cmd/gora chat --config $(CONFIG)
...
wire: ## 重新生成 internal/wired/wire_gen.go（改动 ProviderSet 后必跑）
	go run github.com/google/wire/cmd/wire ./internal/wired
